...
import { fetchWordOfTheDay, shouldFetchWordOfTheDay } from '../../services/word-of-the-day';
...
function HomePage() {
  ...
  useEffect(() => {
    ...
    async function getInitialCard() {
      ...
      try {
        ...
        console.error(err);
        setError('There was an error while loading a card from the deck');
      } finally {
        setLoadingText(null);
      }
    }
  ...
  return (
    <div className='card'>
      ...
      {error && <p>{error}</p>}
      {
        card && (mode === 'question' ? <QuestionCard question={card.question} /> : <p>{ card.answer}</p>)
      }
    </div>
  )
}
